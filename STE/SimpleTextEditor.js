...
function SimpleTextEditor(id, objectId) {
    ...
    this.init = function() {
        if (document.getElementById && document.createElement && document.designMode && (this.browser.ie || this.browser.gecko)) {
            ...
            if(this.textareaValue.length<1){
                this.frame.document.execCommand('formatblock', false, "<p>");
            }
			this.frame.focus();
    ...
    function insertHtmlFromTextarea() {
        try { self.frame.document.body.innerHTML = lockUrls(self.textareaValue.replace(/\[/g, "<").replace(/\]/g, ">")); } catch (e) { setTimeout(insertHtmlFromTextarea, 10); }
    }
